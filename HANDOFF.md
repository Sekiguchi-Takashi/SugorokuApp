...
- 現在バージョン: **v2.0（フェーズ2本格着手：直線スクロール盤面＋ステータス＋公園イベント）**
...
## フェーズ2の残タスク（ここから着手する）
1. イベントマス追加: `Board.CELL_EVENT` に位置を追加し、`showParkEvent()` を汎用化（イベント定義=背景画像・メッセージ・ステータス変化のdata class化を推奨）
2. 満腹/友情が変化するイベント（ごはんマス・なかまマス等）
